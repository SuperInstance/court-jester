...
exports.DialogueSession = void 0;
const turns_js_1 = require("./turns.js");
...
class DialogueSession {
    ...
    constructor(provider, memory, config = {}) {
        ...
        this.memory = memory;
        this.tracker = new turns_js_1.TurnTracker();
        this.config = {
            maxRounds: 5,
     ...
     */
    async runDialogue(agentName, topic, mode, rounds, initialPrompt, additionalContext) {
        ...
        for (let round = 0; round < actualRounds; round++) {
            ...
            if (round < actualRounds - 1) {
                ...
                this.memory.appendEntry(sessionLog, 'agent', 'Continue riffing. Go deeper on the most interesting thread.');
            }
        }
        // Generate summary
     ...
     */
    async singleTurn(userPrompt, contextMessages) {
        const messages = [
            { role: 'system', content: this.config.systemPrompt },
            ...(contextMessages ?? []),
     ...
     */
    async generateSummary(sessionLog, messages) {
...
exports.DialogueSession = DialogueSession;
