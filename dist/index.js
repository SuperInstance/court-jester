...
const jester_js_1 = require("./jester.js");
// ===== MCP Server =====
class CourtJesterMcpServer {
    ...
    clientInfo = null;
    constructor() {
        this.jester = new jester_js_1.CourtJester();
    }
    /**
    ...
    getTools() {
        return [
            {
                name: 'jester_ideate',
                description: 'Generate wild/practical/absurd ideas about a topic. The jester produces rapid-fire ideas at ~$0.02/session. Use when you need novel angles fast.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        topic: {
            ...
            {
                name: 'jester_provoke',
                description: 'Attack an idea to find its weaknesses. The jester plays contrarian to stress-test your thinking. Choose intensity from mild (constructive) to savage (brutal).',
                inputSchema: {
                    type: 'object',
                    properties: {
                        idea: {
            ...
            {
                name: 'jester_riff',
                description: 'Free-associate on a topic across multiple turns. The jester lets its mind wander at max temperature. Best for creative block or exploring a space without preconceptions.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        topic: {
            ...
            {
                name: 'jester_sharpen',
                description: '"Iron sharpens iron" — have the jester find logical gaps, brittle assumptions, and hidden weaknesses in an argument. Returns both weaknesses AND strengths.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        argument: {
            ...
            },
            {
                ...
                description: 'Run N rounds of multi-turn dialogue about a topic, then summarize what emerged. The jester can explore (try different angles), debate (play contrarian), or what-if (twist assumptions). Returns a summary + full transcript.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        topic: {
            ...
            },
            {
                ...
                description: 'Push a dialogue or ideation session to a PLATO room. Uses HTTP (REST) or git (filesystem). Defaults to HTTP.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        room: {
            ...
            },
            {
                ...
                description: 'Pull context from a PLATO room to prime the jester for ideation. Returns recent tiles/commits that can be used as grounding context.',
                inputSchema: {
                    type: 'object',
                    properties: {
                        room: {
     ...
     */
    async handleToolCall(toolName, args, base) {
        try {
            switch (toolName) {
                case 'jester_ideate': {
                ...
                case 'jester_springboard': {
                    const result = await this.jester.springboard({
                        topic: args.topic,
                        rounds: args.rounds ?? 5,
                        style: args.style ?? 'explore',
                            ...
                            content: [
                                {
                                    type: 'text',
                                    text: result.success
                                        ? `Pulled ${result.tiles?.length ?? result.entries?.length ?? 0} items from PLATO room "${args.room}"`
                                        : `❌ Failed: ${result.error ?? 'Unknown error'}`,
                                },
                            ],
                        },
